...
class Makecommerce {
    ...
    constructor(shop_id, publishable_key, secret_key, test_env) {
        ...
        this.env_urls.gateway_get = test_env ? 'https://payment-test.maksekeskus.ee/pay/1/link.html' : 'https://payment.maksekeskus.ee/pay/1/link.html';
    ...
    /**
     * 
     * @param {GatewayPayment} GatewayPayment 
     */
    createPaymentGatewayUrl(gatewayPayment) {
        const gatewayUrl = new URL(this.env_urls.gateway_get);
        gatewayUrl.searchParams.append('shop', this.shop_id);
        const parameters = gatewayPayment.getUriParameters();
        return gatewayUrl + "&" + parameters;
    }

    isPaymentComplete(request) {
        return this.isStatus(request, STATUS_COMPLETED);
    }

    isPaymentCancelled(request) {
        return this.isStatus(request, STATUS_CANCELLED);
    }

    isStatus(request, status) {
        try {
            const requestData = this.extractRequestData(request);
            return requestData.status === status;
        } catch (e) {
            return false;
        }
    }
